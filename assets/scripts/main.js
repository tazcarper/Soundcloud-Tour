...
$(document).ready(function() {
  ...
  buildList = function() {
    $('.startText').hide();
    $('.animation, .loadingCopy').show();
    if (localStorage.songkickArtists) {
      var retrievedObject = JSON.parse(localStorage.getItem('songkickArtists'));
      ...
      $.getJSON(user + '/followings' + clientID + '&limit=200&offset=8000', function(following) {
        ...
        // save soundcloud artists array
        localStorage.setItem('soundcloudUserInfo', JSON.stringify(user_data));
        ...
        $.each(user_data.SC_artists, function(ke, va) {
          $.getJSON("https://api.soundcloud.com/users/" + va[0] + "/web-profiles" + clientID, function(profile) {
            ...
            if (profileLimit == user_data.SC_artists.length) {
              ...
              // Save token for songkick artists array
              localStorage.setItem('songkickArtists', JSON.stringify(user_data.songkick));
  ...
  checkSongKick = function(ids) {
    ...
    $.each(ids, function(songkickArtist, songkickID) {


      $.getJSON("https://api.songkick.com/api/3.0/artists/" + songkickID[0] + "/calendar.json?apikey=NyHcJHQPyD0gM5Ec&jsoncallback=?", function(data) {
        console.log(data.resultsPage.results);

        $.each(data.resultsPage.results, function(key, evt) {
          //console.log(key);
          //console.log(evt);
          $.each(evt, function(ka, val) {
            console.log(val);
            var event_detail = {
              concert_id: val.id,
              concert_avatar: songkickID[1],
              artist_name: songkickID[2],
              ...
              concert_age: val.ageRestriction,
        ...
        songkicklimit++;
        console.log('songkick limit = ' + songkicklimit);
        loadingStatus.html(songkickArtist + 1 + ' / ' + ids.length);
        if (songkicklimit == ids.length) {
  ...
  var showList = function(list) {
    ...
    $.each(list, function(concertItem, concertItemID) {
      convertTime(concertItemID);

      // build lineup
      var theLineup = [];

      // loop through each band in lineup
      $.each(concertItemID.concert_lineup, function(key, band) {
        theLineup.push(band.displayName);
      });

      // create new lineup list of artists
      var artists = theLineup.join(' - ');
      console.log(concertItemID);
      // change avatar to 300x300. Large is 100x100. Can go up to 500x500 with -t500x500
      //concertItemID.concert_avatar = concertItemID.concert_avatar.replace('-large', '-t300x300');
      concertItemID.concert_avatar = concertItemID.concert_avatar.replace('-large', '-t300x300');

      // name of show
      var concert = '<h2>' + concertItemID.concert_name + '</h2>';

      // Type of Concert
      var typeOfConcert = '<div class="type">';
      if (concertItemID.concert_type == 'Festival') {
        typeOfConcert = '<div class="type festival">';
      }

      // Concert or festival
      var concertString = typeOfConcert + '<p>' + concertItemID.concert_type + '</p></div>';

      // City of concert
      var location = '<h2 class="locationText">' + concertItemID.concert_city + '</h2>';

      // convert dates to readable format
      var convertedDate = convertDates(concertItemID.concert_date);

      // collapse ID
      var collapseID = concertItemID.concert_id + concertItemID.artist_name.replace(/[^A-Za-z0-9]/g, '_');

      if (concertItemID.concert_venue_uri == null) {
        //console.log('dunno venue')
        concertItemID.concert_venueName = '';
      }
      // build event and append to DOM

      // first layout
      var anEvent = '<div class="col-sm-12 anEvent" data-miles="' + concertItemID.miles + '">' + concertString + '<div class="row">' +
        '<div class="col-sm-2 col-xs-4 avatar" data-toggle="collapse" href="#' + collapseID + '" aria-expanded="true" aria-controls="' + collapseID + '">' +
        '<div class="captionContainer"><img src="' + concertItemID.concert_avatar + '" alt="' + concertItemID.artist_name + '"><div class="caption disabled"></div></div></div>' +
        '<div class="col-sm-10 col-xs-8 info"><div class="row"><div class="col-sm-8 who ">' +
        '<h1>' + concertItemID.artist_name + '</h1>' + concert + location + '<h3 class="where_venue"><a target="_blank" href="' + concertItemID.concert_venue_uri + '">' + concertItemID.concert_venueName + '</a></h3></div>' +
        '<div class="col-sm-1 col-xs-6 where">' +
        '</div>' +
        '<div class="col-sm-3 col-xs-12 tickets"><h3>' + convertedDate + '</h3><h3 class="concert_timeing">' + concertItemID.concert_time + '</h3>' +
        '<a href="' + concertItemID.concert_ticket + '" target="_blank"><button class="btn btn-default tickets">Get Tickets</button></a>' +
      ...
      var anEvent_2 = '<div class="col-md-6 anEvent">' + concertString + '<div class="row"><div class="innerPad">' +
        ...
        '<img src="' + concertItemID.concert_avatar + '" alt="' + concertItemID.artist_name + '"></div>' +
        '<div class="col-md-4 col-xs-8">' +
        '<h1>' + concertItemID.artist_name + '</h1><h3>' + convertedDate + '</h3><h3>' + concertItemID.concert_time + '</h3></div>' +
        '<div class="col-md-4 col-xs-3">' + location +
        '</div><a class="buyTicket" href="' + concertItemID.concert_ticket + '" target="_blank"><button class="btn btn-default tickets">Find Tickets</button></a></div></div></div>';

      finalList = finalList.concat(anEvent);
      limit++;
      ...
      if (limit == list.length) {
        ...
        $('.tourList').append($.parseHTML(finalList));
